...
//                                orientStr(_pin->orient()));
        ...
        case defrGcellGridCbkType : 
            ...
            {
                Gcg gcgrid;
                gcgrid.direction = (gcg->macro()[0] == 'X') ? HORIZONTAL : VERTICAL;
                gcgrid.orig = (int)gcg->x();
                gcgrid.stepNum = (int)gcg->xNum();
                gcgrid.stepSize = (int)gcg->xStep();
                ckt->gcgs.push_back(gcgrid);
            }
            break;
