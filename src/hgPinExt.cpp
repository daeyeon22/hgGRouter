...
void HGR::pin_extension()
{
    ...
    for(int i=0; i < num_pins(); i++)
    {
        ...
        if(rou->pinAccessPool[pinid].size() == 0)
        {
            cout << get_pin(pinid)->name << " -> No candidate!" << endl;
            exit(0);
        }
        else
        {
            //cout << get_pin(pinid)->name << " -> #candidate " << rou->pinAccessPool[pinid].size() <<  endl;
        }
...
bool HGR::pin_extension(int _pin)
{
    ...
    for(auto it_e : pin->envelope)
    {
        ...
        else
        {
            ...
            // Pin Extension For I/O or Block Cell
            


            SegRtree* tree = &db->preferred[l1];
            for(SegRtree::const_query_iterator it = tree->qbegin(bgi::nearest(convert(envelope),1));
                    it != tree->qend(); it++)
            {
                int d2 = preferred(l2);
                int accessTrack = it->second;
                Point<int> hp((envelope.ll.x + envelope.ur.x)/2, (envelope.ll.y + envelope.ur.y)/2);
                Ext ext;
                if(make_extension(_pin, l1, l1, accessTrack, "", hp, ext))
                {
    ...
    if(candidates.size() != 0)
    {
        ...
        for(int j=0; j < (int)candidates.size(); j++)
        {
            if(j==0)
            {
                ...
                rou->add_extension(pin->id, &candidates[j]);
            }
            else if(candidates[j].penalty == 0)
            {
                rou->add_extension(pin->id, &candidates[j]);
            }
        }
...
bool HGR::make_extension(int _pin, int _l1, int _l2, int _track, string _viaType, Point<int> _hp, Ext &_outExt)
{
    ...
    if(_outExt.track == OFF_TRACK)
    {
        //cout << "Hit Point is Off-track!" << endl;
        

        SegRtree* tree = &db->preferred[_l2];

        
        for(SegRtree::const_query_iterator it = tree->qbegin(bgi::nearest(convert(_hp), 1)); it != tree->qend(); it++)
        {
            Track* track = get_track(it->second);
            

            _outExt.track = track->id;

            Point<int> ap;

            ap.x = is_vertical(_l2) ? track->ll.x : _hp.x;
            ap.y = is_vertical(_l2) ? _hp.y : track->ll.y;

            Rect<int> line = get_line(_hp, ap);
            Rect<int> shape = wire_shape(_l2, line);
            ...
            wire.net = pin->net;
            wire.layer = _l2;
            wire.track = get_track_index(_l2, line);
            wire.width = shape.ur.x - shape.ll.x;
            wire.height = shape.ur.y - shape.ll.y;
            wire.line = line;
            wire.metalShape = shape;
            ...
            _outExt.track = track->id;
            _outExt.access = ap;
            _outExt.wires.push_back(wire);
            _outExt.wireConn.push_back(false);
            _outExt.penalty += drc->check_violation_metal(pin->net, _l2, shape);
            break;
        }

        if(_outExt.track == OFF_TRACK)
        {
            cout << "Still offtrack... ????!?" << endl;
            exit(0);
        }
...
{
    return &rou->pinAccessExt[_pin];
}

bool HGR::is_extended(int _pin)
{
    return (rou->pinAccessExt.find(_pin) == rou->pinAccessExt.end()) ? false : true;
}
