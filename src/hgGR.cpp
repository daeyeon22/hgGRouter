...
bool HGR::route_all()
{
    ...
    for(int iterCount = 0; iterCount < 5; iterCount++)
    {
        ...
        for(auto i : _nets)    
        {
            ...
            { 
                if(check_congested_region(i))
                {
                    ...
                    backup[i] = rou->topologies[i];
                    rou->topologies[i].clear();
                }
                else
                {
                    continue;
        ...
        {
            Gcell* gcell = grid->gcell(i);
            int edgeCap = gcell->edgeCap;
        ...
        if(prevCongested < levelCongested)
        {
            for(auto it : backup)
            {
                rou->topologies[it.first] = it.second; //_backup;
            }
            break;
        }
...
bool HGR::check_congested_region(int netid)
{
    ...
    for(int i=0; i < (int)_tp->segments.size(); i++)
    {
        ...
        Gcell* g1 = grid->gcell(_s->g1);
        Gcell* g2 = grid->gcell(_s->g2);
...
bool HGR::route(int _net, int _trial)
{
    ...
    net->get_routing_space(bufferDistance, rSpace);
...
double HGR::HeapNode::get_act_cost(int _prevNode, double _prevActCost, set<int> &_rGuide)
{
    ...
    double _act = 
        ...
        + grid->overflow(g2);
    */
    double wl = grid->wirelength(g1, g2);
    double sg = grid->segment(g1, g2);
    double og = grid->offguide(g2, _rGuide);
    double np = grid->npref(g1, g2);
    double of = grid->overflow(g2);

    double _act = _prevActCost + wl + sg + og + np + of;

    //cout << _prevActCost << " wl: " << wl << " sg: " << sg << " og: " << og << " np: " << np << " of: " << of << " total: " << 
    //    _prevActCost + wl + sg + og + np + of << endl;
...
bool HGR::route_pin_to_tp(int _net, int _pin, set<int> &_rGuide, set<int> &_rSpace)
{
    ...
    else
    {
        ...
        //
        //Gcell* c1 = grid->gcell(n);
        //cout << "Extension -> Gcell[n] : " << n << " (" << c1->x << " " << c1->y << " " << c1->z << ")" << endl;
    ...
    if(heap.size() == 0)
    {
        cout << "[1] No Element into Heap!" << endl;
        exit(0);
    }
    ...
    while(!heap.empty())
    {
        ...
        if(n1 == minNode)
        {
            Gcell* c1 = grid->gcell(n1);
            Gcell* c2 = grid->gcell(nodes[n1].backtrace);

            if(c1->z != c2->z)
            {
                minNode = nodes[n1].backtrace;
            }
        ...
        for(size_t i=0; i < 4; i++)
        {
            ...
            //cout << "Previous Act Cost : " << nodes[n1].act << endl;
            //cout << "Current  Act Cost : " << act << " (ass : " << grid->assign[n2] << " his: " << grid->history[n2] << ")" << endl; 
...
bool HGR::route_twopin_net(int _net, int _p1, int _p2, set<int> &_rGuide, set<int> &_rSpace)
{
    ...
    else
    {
        ...
        //Gcell* c1 = grid->gcell(n1);
        //Gcell* c2 = grid->gcell(n2);
        //cout << "Extension -> Gcell[n1] : " << n1 << " (" << c1->x << " " << c1->y << " " << c1->z << ")" << endl;
        //cout << "Extension -> Gcell[n2] : " << n2 << " (" << c2->x << " " << c2->y << " " << c2->z << ")" << endl;
    ...
    while(!heap.empty())
    {
        ...
        grid->coord(n1, x1, y1, z1);
        //cout << "Current Node : " << n1 << " (" << x1 << " " << y1 << " " << z1 << ")" << endl;
...
void HGR::subnet_segmentation(int _net, int _p1, int _p2, vector<int> &_gcells)
{
    ...
    {
        Gcell* c = grid->gcell(_gcells[i]);
        cout << boost::format("%s (%d %d)") % c->z % c->x % c->y;
        if(i == 0 && _p2 != INT_MAX)
        {
            cout << boost::format(" -> [%s]") % get_pin(_p2)->name;
        }

        if(i == _gcells.size()-1)
        {
            cout << boost::format(" -> [%s]") % get_pin(_p1)->name;
        }
    ...
    else
    {
        ...
        for(size_t i=1; i < _gcells.size(); i++)
        {
            ...
            if(diffZ | diffXY)
            {
                //cout << "g1 : (" << c1->x << " " << c1->y << " " << c1->z << ")" << endl;;
                //cout << "g2 : (" << c2->x << " " << c2->y << " " << c2->z << ")" << endl;;
                //cout << "g3 : (" << c3->x << " " << c3->y << " " << c3->z << ")" << endl;;
                //cout << "g1 - g2" << endl;
